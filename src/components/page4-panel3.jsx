  ...
  return (
    <div className="">
      ...
      <table className="table-auto text-sm text-gray-500">
        <thead>
          <tr>
            <th className="font-normal border-r-[1px] border-b-[1px]  border-gray-400 box-border p-3">
              Distance
            </th>
            <th className="font-normal border-b-[1px]  border-gray-400 box-border p-3 text-left">
              Name of the place
            </th>
          </tr>
        </thead>
        <tbody>
          {landmarks?.map(({ distance, placeName }, index) => (
            <tr key={index} className="border-gray-400 box-border">
              <td className="border-r-[1px] border-b-[1px]  border-gray-400 box-border p-3">
                {distance} km
              </td>
              <td className="border-b-[1px]  border-gray-400 box-border p-3">
                {placeName}
              </td>
            </tr>
          ))}
      ...
      <div className="flex mb-2">
        ...
        <div className="flex flex-col">
          <div className="text-md">
            Until {until ?? "24"} hours from check in
          </div>
    ...
    <>
      <div className="grid sm:grid-rows-2 sm:grid-flow-col sm:gap-4 w-full h-[250px]">
        <div className="row-span-2 col-span-3 relative">
          ...
          <div
            className="sm:hidden absolute w-full h-full z-10 bg-black bg-opacity-25 hover:bg-opacity-20"
        ...
        {images
          ...
          .map((image, index) => (
            <div className="max-sm:hidden sm:col-span-2 relative" key={index}>
              <Image
                src={image.image}
                alt="grid pictures"
                fill
...
const BedTypePanel = ({ bedTypes }) => {
  return (
    <div className="grid sm:grid-cols-3 w-full rounded-md mt-4 overflow-hidden border-[1px] border-gray-200 box-border">
      {bedTypes?.map(
        ({ people, image, type, view, ViewIcon, beds, BedIcon, desc }, ind) => (
          <div
            className="flex flex-col p-2 border-[1px] m-2 border-gray-200"
            key={ind}
          >
      ...
      <ul className="grid grid-cols-6 gap-2 max-xs:grid-cols-4 md:max-lg:grid-cols-4 mb-5 max-sm:mt-4">
        {options.map((option, ind) => {
          ...
          return (
            <li
              key={ind}
              className="mx-2 flex flex-col justify-center items-center sm:grow"
            >
              <div
                key={ind}
                className="border-gray-400 border-[1px] rounded-lg px-3 py-2"
              >
...
export default function Page4Panel1({
  id,
  images,
  hotelName,
  address,
        ...
        <div className="flex max-sm:flex-col">
          ...
          <div className="flex flex-col sm:py-0 sm:ml-12 grow">
            ...
            <ul className="text-sm list-disc ml-3">
              {importantNotes?.split("*").map((note, index) => {
