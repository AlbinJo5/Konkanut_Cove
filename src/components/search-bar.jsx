            ...
            {dropdowns.map((dd,ind)=>
                (()=>{
                    if(["Check In","Check Out"].includes(dd)) return(
                        <div className="flex flex-col max-md:mb-3 w-full px-3 relative" key={ind}>
                            <div className="flex justify-center items-center mt-2" onClick={()=>setIndex(ind)}>
                                <div className="font-bold text-green-800 grow">{dd}</div>
                                <Calendar/>
                            </div>
                            {index===ind ?
                            <>
                                <div className="fixed z-0 w-full h-screen inset-0" onClick={()=>setIndex(null)}></div>
                                <div className="absolute z-20">
                                    <div className="p-3 bg-white">
                                        ...
                                        <ReactDatePicker key={ind} selected={startDate} onChange={(date) => setStartDate(date)} className="w-[150px] rounded-md border-2 border-gray-400"/>
                                    </div>
                                </div>
                            </>:null}
