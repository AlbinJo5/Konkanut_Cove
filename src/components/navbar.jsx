    ...
    <nav className="flex flex-col w-full h-20 apply-gradient items-center">
      <div className="flex w-full h-full items-center">
        <div className="flex-grow ml-6 md:ml-16">
          <Image src="/assets/images/logo.svg" alt="" width="120" height="80"/>
        </div>

          <div className="flex h-full max-sm:hidden ">
            {buttonList.map(({name,url},ind)=>
              <a href={url} key={ind} className={`h-full flex items-center justify-center px-4 sm:mr-4 lg:mr-10 text-sm font-sans text-green-800 hover:border-green-700 hover:border-b-4 hover:bg-white hover:bg-opacity-25  ${selectedIndex===ind && selectedIndexClasses}`}>{name}</a>
            )}
          </div>
